...
4. Verify Hyperledger Fabric ledger recording

### Blockchain Integration
CountyPay uses **Hyperledger Fabric** (mock ledger) for immutable payment records.

**How it works:**
- After successful M-Pesa payment, transaction is recorded on Fabric ledger via `fabricService`
- Each payment gets a unique `paymentId` and `timestamp` stored on the ledger
- Payments are queryable and verifiable through the blockchain API
